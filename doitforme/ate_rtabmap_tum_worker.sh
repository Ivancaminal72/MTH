...
for ((i=0;i<${#seq_a[@]};++i)); do
	...
	for ((j=0;j<${#dot_a[@]};++j)); do
		...
		python ~/workspace/metrics_eval/evaluate_ate.py --verbose ./groundtruth.txt ./${in_name} --plot plot.${dot_a[j]}.png | tee -a $out_file

#		printf "\n${seq_a[i]};" | tee -a $out_file		
#		printf "${dot_a[j]}.od;" | tee -a $out_file

#		#With loop closure
#		python ~/workspace/metrics_eval/evaluate_ate.py --verbose ./groundtruth.txt ./${in_name}.od --plot plot.${dot_a[j]}.od.png | tee -a $out_file
