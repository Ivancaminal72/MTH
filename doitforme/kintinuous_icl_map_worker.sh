...
for ((i=0;i<4;++i)); do
	...
	for ((j=0;j<${#dot_a[@]};++j)); do
		...
		srun --x11 --mem=16GB -c4 --gres=gpu:maxwell:1 vglrun ./Kintinuous -v ../../vocab.yml.gz -l $path/$i/$version/log_5000.klg ${test_a[j]} -c $path/calib.txt -m -f -cw 1 > $path/$i/$version/worker/out.${dot_a[j]}.txt 2> $path/$i/$version/worker/err.${dot_a[j]}.txt
		...
		#With loop closure
		srun --x11 --mem=16GB -c4 --gres=gpu:maxwell:1 vglrun ./Kintinuous -v ../../vocab.yml.gz -l $path/$i/$version/log_5000.klg ${test_a[j]} -od -c $path/calib.txt -m -f -cw 1 > $path/$i/$version/worker/out.${dot_a[j]}.od.txt 2> $path/$i/$version/worker/err.${dot_a[j]}.od.txt
