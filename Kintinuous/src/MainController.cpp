...
bool MainController::setup()
{
    ...
    trackerInterface = new TrackerInterface(logRead, depthIntrinsics, scale);
...
int MainController::mainLoop()
{
    ...
    {
        // cout<<endl<<i<<"----> "<<systemComponents.at(i)->getThreadIdentifier()<<endl;
        // if(i==2) continue;
        // if(i==1) continue;
...
void MainController::loadCalibration()
{
    ...
    if(calFile.length() > 0)
    {
        ...
        if(extension == ".xml" || extension == ".yml"){
            ...
            std::getline(file, line);
            if(!file.eof()) scale = 1.0/std::stod(line);
